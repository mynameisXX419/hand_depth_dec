...
#             rate = np.nan
#         else:
#             interval = tp[k] - prev_tp
#             rate = 60000.0 / interval if interval > 0 else np.nan
...
# def main():
#     print("=== Dual Peak Alignment + Sync + Rate ===")

#     tP, xP, _ = load_pressure_series()
...
#     pairs = pair_peaks(tp, ip, tv, iv)

#     print(f"\nTotal matched pairs: {len(pairs)}\n")

#     # ================= 每对逐对打印 =================
#     for p_ in pairs:
#         print(f"[PAIR {p_['pair_id']:02d}]")
#         print(f"  Pressure peak @ {p_['pressure_t_ms']/1000:7.3f} s")
#         print(f"  Vision   peak @ {p_['vision_t_ms']/1000:7.3f} s")
#         print(f"  Sync Δt        = {p_['sync_dt_ms']:7.1f} ms")
#         print(f"  Interval (P-P) = {p_['interval_ms']:7.1f} ms")
#         print(f"  Rate           = {p_['rate_cpm']:7.1f} cpm\n")

#     # ================= 时间同步统计 =================
#     dt = np.array([p_["sync_dt_ms"] for p_ in pairs])
#     abs_dt = np.abs(dt)

#     print("=== Temporal Synchronization Performance ===")
#     print(f"Mean |Δt| = {abs_dt.mean():.1f} ms")
#     print(f"Std  Δt  = {dt.std():.1f} ms")
#     print(f"Max  |Δt| = {abs_dt.max():.1f} ms")
#     print(f"|Δt| ≤ 50 ms  ratio = {np.mean(abs_dt <= 50)*100:.1f}%")
#     print(f"|Δt| ≤ 100 ms ratio = {np.mean(abs_dt <= 100)*100:.1f}%")

#     # ================= 频率统计 =================
#     rates = np.array([p_["rate_cpm"] for p_ in pairs if not np.isnan(p_["rate_cpm"])])

#     print("\n=== Compression Rate Statistics ===")
#     print(f"Mean CCR = {rates.mean():.1f} cpm")
#     print(f"Std  CCR = {rates.std():.1f} cpm")
#     print(f"Min  CCR = {rates.min():.1f} cpm")
#     print(f"Max  CCR = {rates.max():.1f} cpm")

#     pd.DataFrame(pairs).to_csv(
#         "peak_pairs_with_sync_and_rate.csv", index=False
#     )
