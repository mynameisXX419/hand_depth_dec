...
import pressure_detector_6 as pressure_detector
...
def get_click_wav():
    ...
    if CLICK_WAV is None:
        CLICK_WAV = sa.WaveObject.from_wave_file(
            os.path.join(os.path.dirname(__file__), "assets", "click.wav")
        )
    return CLICK_WAV


def play_cpr_30_voice_counts(bpm):
    global voice_thread_started, cpr_30_completed

    bpm = max(100, min(120, float(bpm)))
    interval = 60.0 / bpm

    t0 = time.perf_counter()
    print(f"[CLICK] Start CPR 30 @ {bpm:.1f} BPM")

    for i in range(28):
        get_click_wav().play()
        time.sleep(interval)

    print("[CLICK] CPR 30 finished")

    with voice_thread_lock:
        voice_thread_started = False
        cpr_30_completed = True   # ⭐ 关键

# ===== CPR 30次语音节拍控制 =====
voice_thread_started = False
cpr_30_completed = False  
voice_thread_lock = threading.Lock()
def start_cpr_voice_thread(bpm):
    global voice_thread_started, cpr_30_completed

    with voice_thread_lock:
        if voice_thread_started or cpr_30_completed:
            return

        voice_thread_started = True

        t = threading.Thread(
            target=play_cpr_30_voice_counts,
            args=(bpm,),
            daemon=True
        )
        t.start()
...
def prune_pressure_buffer(now_ms):
    while vision_press_buffer:
        _, _, _, t_ms, _, _, _, _ = vision_press_buffer[0]
...
def fetch_and_dispatch_pressure_peaks():
    ...
    for p in new_peaks:
        ...
        # ⭐⭐ 立刻把“回弹充分性”发给 UI（事件型）
        recoil_ok_ui = (p.get("recoil_mode") == "baseline")

        ui_sock.sendto(
            json.dumps({
                "type": "recoil_status",
                "idx": p["global_idx"],
                "recoil_ok": recoil_ok_ui,
            }).encode(),
            UI_SOCKET_PATH
        )

    # 控制 buffer 长度
    while len(vision_press_buffer) > 100:
        vision_press_buffer.popleft()
...
def filter_pressure_peaks(vis_time_ms, is_vision_peak=True):
    ...
    for item in list(vision_press_buffer):
        _, g_idx, p_val, t_ms, _, _, _, _ = item
    ...
    while True:
        ...
        try:
            ...
            for item in list(pressure_only_buffer):
                l_idx, g_idx, p_val, t_ms, frame_256, bpm, recoil_ok, recoil_mode = item
                ...
                last_press_bpm = bpm
                # ===== 第一次检测到按压 → 启动 30 次语音节拍（独立线程）=====
                start_cpr_voice_thread(last_press_bpm or 110)
        ...
        if t == "peak":
            ...
            # --- 5. 拿到 pressure ---
            _, _, p_val, _, frame_256, press_bpm, recoil_ok, recoil_mode = matched[0]
            ...
            ui_sock.sendto(json.dumps({
                ...
                "press": last_press_val,
                "bpm": last_press_bpm,
                "recoil_ok": (recoil_mode == "baseline")   # ✅ 正确
            }).encode(), UI_SOCKET_PATH)
