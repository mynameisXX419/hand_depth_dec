...
MIN_PRESS_DURATION_MS = 250       # 最短按压时间
MIN_INTER_PRESS_MS    = 450       # 相邻按压最小间隔
RELEASE_RATIO = 0.90              # ⭐ 回弹比例（关键新增）

# ================= 全局数据 =================
_samples = []   # {idx, host_ms, val_raw, val_filt}
_peaks   = []   # {idx, val, t_ms, bpm, frame}
    ...
    while _running:
        ...
        with _lock:
            ...
            if _state == STATE_IDLE:
                ...
                # ===== 峰后回弹确认（关键改动）=====
                if (recv_ms - _press_start_t >= MIN_PRESS_DURATION_MS) and \
                   (val_filt < RELEASE_RATIO * _peak_max_val_filt):
                    ...
                    _peaks.append({
                        "idx": _peak_max_idx,
                        "val": int(_peak_max_val_raw),  # ⭐ 用 raw
                        "t": _peak_max_t,
                        "bpm": bpm,
...
def export_peaks_csv(path="pressure_peaks_test.csv"):
    with open(path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["idx", "peak_val", "t_ms", "bpm"])
        for p in _peaks:
            w.writerow([p["idx"], p["val"], p["t"], p.get("bpm", None)])
