...
class FusionMonitor(QWidget):
    ...
    def _init_ui(self):
        ...
        s.addWidget(QLabel("Recoil:"), 2, 0)
        s.addWidget(self.lbl_recoil, 2, 1)

        s.addWidget(QLabel("Occlusion Seq:"), 3, 0)
        s.addWidget(self.lbl_occl, 3, 1)
    ...
    def _set_recoil(self, recoil_ok):
        self.recoil_ok = recoil_ok

        if recoil_ok is True:
            self.lbl_recoil.setText("OK")
            self.lbl_recoil.setStyleSheet(
                "color:green; font-weight:bold; font-size:14px;"
            )
        elif recoil_ok is False:
            self.lbl_recoil.setText("INSUFFICIENT")
            self.lbl_recoil.setStyleSheet(
                "color:red; font-weight:bold; font-size:14px;"
            )
        else:
            self.lbl_recoil.setText("-")
            self.lbl_recoil.setStyleSheet(
    ...
    def _handle_event(self, msg):
        ...
        if t == "peak":
            ...
            if "recoil_ok" in msg:              # ⭐ 关键
                self._set_recoil(msg["recoil_ok"])

        elif t == "recoil_status":
            self._set_recoil(msg.get("recoil_ok"))
